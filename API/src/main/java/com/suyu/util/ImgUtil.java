...
public class ImgUtil {
    ...
    public static void main(String[] args){
        ...
        String name = "D:/sign/nie.png";

        ImgUtil o = new ImgUtil(100,100,100,100);
        o.setSrcpath(name);
        o.setSubpath("D:/sign/2.jpg");
        try {
            o.cut() ;
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static BufferedImage spin(int degree,byte[] bytes) throws Exception {
        int swidth = 0; // 旋转后的宽度
        int sheight = 0; // 旋转后的高度
        int x; // 原点横坐标
        int y; // 原点纵坐标
        BufferedImage bi = ImageIO.read(new ByteArrayInputStream(bytes)); // 读取该图片
        // 处理角度--确定旋转弧度
        degree = degree % 360;
        if (degree < 0)
        ...
        return spinImage = op.filter(bi, spinImage);
//        File sf = new File("D:/Work", "test9" + "." + "png");
